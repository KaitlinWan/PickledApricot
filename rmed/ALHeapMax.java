...
public class ALHeapMax
{
  ...
  *****************************************************/
  public Integer removeMax()
  {
    ...
    //largest is at index 0, so replace it with last element in heap
    Integer toReturn = _heap.get(0);
    swap(0, _heap.size() - 1);
    _heap.remove(_heap.size() - 1);

    //now we're in a state that the "max" element might not be min.
    //swap until it's in the right place

    int index = 0;

    //while the element has children
    while (maxChildPos(index) != -1) {

      //store the minChildPos so you don't have to call it a lot
      int mcp = maxChildPos(index);

      //if the smallest child is less than the parent, shouldSwap = true. Else shouldSwap = false;
      boolean shouldSwap = (maxOf(_heap.get(index), _heap.get(mcp)) == _heap.get(mcp)) ? true : false;
  ...
  *****************************************************/
  private int maxChildPos( int pos )
  {
    ...
    int maxPos = (maxOf(l, r) == r) ? rIndex : lIndex;
